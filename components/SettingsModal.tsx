...
export const SettingsModal: React.FC<SettingsModalProps> = ({
  ...
  const handleImport = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    showModal({
      ...
      title: "Merge Data?",
      message:
        "This will merge the backup into your current library. Existing data will be preserved.",
      confirmText: "Merge",
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm animate-fade-in">
      <div className="w-full max-w-md bg-bg-surface border border-border-light rounded-2xl p-6 shadow-2xl animate-slide-up transform transition-all max-h-[90vh] overflow-y-auto scrollbar-hide">
        ...
        {/* Accent Color */}
        <div
          className="mb-6 opacity-0 animate-slide-up relative z-20"
        ...
        >
          ...
          <div className="flex items-center gap-3">
            <CustomColorPicker
              color={localConfig.themeAccent}
              onChange={(c) => handleChange("themeAccent", c)}
            />
            <input
              type="text"
          ...
          <div className="mt-4 pt-4 border-t border-border-light">
            <ConfirmationToggle
              label="Light Mode"
              checked={localConfig.lightMode}
              onChange={(v) => handleChange("lightMode", v)}
            />
          </div>
        </div>
        ...
        {/* API Key */}
        <div
          className="mb-6 opacity-0 animate-slide-up relative z-10"
            ...
            <div className="space-y-2">
              <div className="flex gap-2">
                <input
                  ...
                  onKeyDown={(e) =>
                    e.key === "Enter" && handleValidateAndSave()
                  }
                />
                <button
        ...
        >
          ...
          <div className="space-y-1 bg-bg-element/30 rounded-xl p-4 border border-border-light">
            <ConfirmationToggle
              ...
              checked={localConfig.confirmations?.deleteProject ?? true}
              onChange={(v) => handleConfirmationChange("deleteProject", v)}
            />
            <ConfirmationToggle
              label="Delete Message"
              checked={localConfig.confirmations?.deleteMessage ?? true}
              onChange={(v) => handleConfirmationChange("deleteMessage", v)}
            />
            <ConfirmationToggle
              label="Delete Image"
              checked={localConfig.confirmations?.deleteImage ?? true}
              onChange={(v) => handleConfirmationChange("deleteImage", v)}
            />
            <ConfirmationToggle
              label="Regenerate & Edit"
              checked={localConfig.confirmations?.regenerate ?? true}
              onChange={(v) => handleConfirmationChange("regenerate", v)}
            />
            <ConfirmationToggle
              label="Fork Conversation"
              checked={localConfig.confirmations?.fork ?? true}
              onChange={(v) => handleConfirmationChange("fork", v)}
            />
          </div>
        </div>
        ...
        {/* Shortcuts */}
        <div
          className="mb-6 opacity-0 animate-slide-up hidden md:block"
          style={{ animationDelay: "0.5s" }}
        >
          <button
            onClick={() =>
              showModal({ type: "shortcuts", title: "Keyboard Shortcuts" })
            }
          ...
          >
            ...
            <div className="flex items-center gap-1">
              <span className="text-[10px] bg-bg-surface px-1.5 py-0.5 rounded border border-white/10 text-text-tertiary">
                Alt
              </span>
              <span className="text-[10px] bg-bg-surface px-1.5 py-0.5 rounded border border-white/10 text-text-tertiary">
                /
        ...
        >
          <a
            href="https://github.com/sarricafabio/okobit"
            target="_blank"
          ...
          {progress ? (
            ...
            <div className="space-y-3">
              <div className="flex gap-2">
                ...
                <label
                  className={`flex-1 bg-bg-element border border-dashed border-text-tertiary text-text-secondary hover:border-accent hover:text-text-primary hover:bg-bg-surface rounded-lg py-2 px-4 text-sm font-medium transition-all text-center cursor-pointer ${isProcessing ? "opacity-50 cursor-not-allowed" : ""}`}
                >
                  Import ZIP
                  <input
                    type="file"
        ...
        {/* Reset */}
        <div
          className="pt-4 border-t border-border-light opacity-0 animate-slide-up"
